Shader "CGTestat/GreyscaleNormalShader"
{
	...
	SubShader
	{
		Pass
		{
			...
			fixed4 frag (v2f fragIn) : SV_Target
			{
                ...
                //Greyscale
                float grey = dot(float3(0.3, 0.59, 0.11), color);
        		return grey;//(0.2126*Math.Pow(r, gamma) + 0.7152*Math.Pow(g, gamma) + 0.0722*Math.Pow(b, gamma));
